...
class JointEmbeddingPredictiveModule(nn.Module):
    ...
    def forward(
        ...
        agent_include_mask: Optional[torch.Tensor] = None,
        agent_input_hidden_mask: Optional[torch.Tensor] = None,
        agent_loss_mask: Optional[torch.Tensor] = None,
        ...
        map_include_mask: Optional[torch.Tensor] = None,
        map_input_hidden_mask: Optional[torch.Tensor] = None,
        map_loss_mask: Optional[torch.Tensor] = None,
        map_token_positions: Optional[torch.Tensor] = None,
        pack_hidden_map_tokens_last: bool = False,
        ...
        agent_include_mask = (
            agent_block_mask
            if agent_include_mask is None
            else agent_include_mask.to(device=scene_summary.device, dtype=torch.bool) & agent_block_mask
        )
        agent_input_hidden_mask = (
            torch.zeros_like(agent_include_mask)
            if agent_input_hidden_mask is None
            else agent_input_hidden_mask.to(device=scene_summary.device, dtype=torch.bool) & agent_include_mask
        )
        agent_loss_mask = (
            agent_input_hidden_mask
            if agent_loss_mask is None
            else agent_loss_mask.to(device=scene_summary.device, dtype=torch.bool) & agent_include_mask
        )
        ...
        map_include_mask = (
            map_block_mask
            if map_include_mask is None
            else map_include_mask.to(device=scene_summary.device, dtype=torch.bool) & map_block_mask
        )
        map_input_hidden_mask = (
            torch.zeros_like(map_include_mask)
            if map_input_hidden_mask is None
            else map_input_hidden_mask.to(device=scene_summary.device, dtype=torch.bool) & map_include_mask
        )
        map_loss_mask = (
            map_input_hidden_mask
            if map_loss_mask is None
            else map_loss_mask.to(device=scene_summary.device, dtype=torch.bool) & map_include_mask
        )
        ...
        agent_mask = packed_loss_mask & (token_type_ids == 0)
        map_mask = packed_loss_mask & (token_type_ids == 1)
        ...
        stats = {
            "jepa_cosine": cosine[packed_loss_mask].mean(),
            "masked_fraction": packed_loss_mask.float().sum() / packed_valid_mask.float().sum().clamp_min(1.0),
    ...
    def _build_predictor_tokens(
        ...
        packed_input_hidden_mask: torch.Tensor,
        ...
        return torch.where(packed_input_hidden_mask.unsqueeze(-1), mask_tokens, packed_online)
    ...
    def _pack_tokens_by_graph(
        ...
        agent_include_mask: torch.Tensor,
        agent_input_hidden_mask: torch.Tensor,
        agent_loss_mask: torch.Tensor,
        agent_graph_index: torch.Tensor,
        agent_chunk_ids: torch.Tensor,
        ...
        map_include_mask: torch.Tensor,
        map_input_hidden_mask: torch.Tensor,
        map_loss_mask: torch.Tensor,
        map_graph_index: torch.Tensor,
        map_token_positions: torch.Tensor,
        pack_hidden_map_tokens_last: bool,
    ) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        sequences_online = []
        sequences_target = []
        sequences_valid = []
        sequences_input_hidden = []
        sequences_loss = []
        sequences_type = []
        sequences_chunk_ids = []
        sequences_positions = []
        ...
        for graph_id in range(num_graphs):
            ...
            if graph_agents.any():
                ...
                graph_include = agent_include_mask[graph_agents].reshape(-1) & graph_valid
                graph_input_hidden = agent_input_hidden_mask[graph_agents].reshape(-1)
                graph_loss = agent_loss_mask[graph_agents].reshape(-1)
                ...
                if graph_include.any():
                    graph_online_parts.append(graph_online[graph_include])
                    graph_target_parts.append(graph_target[graph_include])
                    graph_input_hidden_parts.append(graph_input_hidden[graph_include])
                    graph_loss_parts.append(graph_loss[graph_include])
                    graph_type_parts.append(torch.zeros(int(graph_include.sum().item()), dtype=torch.long, device=graph_valid.device))
                    graph_chunk_id_parts.append(graph_chunk_id[graph_include])
                    graph_position_parts.append(graph_position[graph_include])
            ...
            if graph_maps.any():
                ...
                graph_include = map_include_mask[graph_maps] & graph_valid
                graph_input_hidden = map_input_hidden_mask[graph_maps]
                graph_loss = map_loss_mask[graph_maps]
                graph_position = map_token_positions[graph_maps]
                if graph_include.any():
                    graph_online = graph_online[graph_include]
                    graph_target = graph_target[graph_include]
                    graph_input_hidden = graph_input_hidden[graph_include]
                    graph_loss = graph_loss[graph_include]
                    graph_position = graph_position[graph_include]
                    if pack_hidden_map_tokens_last and graph_input_hidden.numel() > 0:
                        visible_indices = torch.nonzero(~graph_input_hidden, as_tuple=False).squeeze(-1)
                        hidden_indices = torch.nonzero(graph_input_hidden, as_tuple=False).squeeze(-1)
                        order = torch.cat([visible_indices, hidden_indices], dim=0)
                        graph_online = graph_online[order]
                        graph_target = graph_target[order]
                        graph_input_hidden = graph_input_hidden[order]
                        graph_loss = graph_loss[order]
                        graph_position = graph_position[order]
                    graph_online_parts.append(graph_online)
                    graph_target_parts.append(graph_target)
                    graph_input_hidden_parts.append(graph_input_hidden)
                    graph_loss_parts.append(graph_loss)
                    graph_type_parts.append(torch.ones(graph_online.shape[0], dtype=torch.long, device=graph_valid.device))
                    graph_chunk_id_parts.append(torch.zeros(graph_online.shape[0], dtype=torch.long, device=graph_valid.device))
                    graph_position_parts.append(graph_position)

            if graph_online_parts:
                graph_online = torch.cat(graph_online_parts, dim=0)
                graph_target = torch.cat(graph_target_parts, dim=0)
                graph_input_hidden = torch.cat(graph_input_hidden_parts, dim=0)
                graph_loss = torch.cat(graph_loss_parts, dim=0)
                ...
                graph_input_hidden = agent_block_mask.new_zeros((0,))
                graph_loss = agent_block_mask.new_zeros((0,))
            ...
            sequences_input_hidden.append(graph_input_hidden)
            sequences_loss.append(graph_loss)
        ...
        if max_tokens == 0:
            ...
            return empty_tokens, empty_tokens, empty_mask, empty_mask, empty_mask, empty_types, empty_chunk_ids, empty_positions
        ...
        packed_input_hidden = agent_block_mask.new_zeros((num_graphs, max_tokens))
        packed_loss = agent_block_mask.new_zeros((num_graphs, max_tokens))
        ...
        for graph_id, (
            ...
            packed_input_hidden[graph_id, :length] = graph_input_hidden
            packed_loss[graph_id, :length] = graph_loss
