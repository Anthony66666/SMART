...
def save_validation_visualization(data, prediction, output_path: Path, title: str, max_agents: int = 0) -> None:
    import matplotlib
...
def _draw_map(ax, data) -> None:
    ...
    for polygon_id in torch.unique(polygon_ids).tolist():
        ...
        ax.plot(points[:, 0], points[:, 1], color="#c7c7c7", linewidth=1.0, alpha=0.8, zorder=1)


def _draw_agents(ax, data, prediction, agent_indices, current_step, hist_steps, av_index, polygon_cls, circle_cls) -> None:
    for agent_index in agent_indices.tolist():
        ...
        if gt.numel() > 0:
            ax.plot(gt[:, 0], gt[:, 1], color=color, linestyle="-", linewidth=linewidth, alpha=0.9, zorder=4)
        if pred.numel() > 0:
        ...
        _draw_agent_shape(ax, current_xy, current_heading, shape, agent_type, color, edge_color, polygon_cls, circle_cls)
