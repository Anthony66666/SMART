...
- Run a SMART-Diffusion GPU smoke with `configs/train/train_scalable_diffusion_local.yaml` after the temporal block diffusion update.
...
- Watch whether target-category-only training, 6-layer decoding, remask sampling, and 4-chunk block rollout reduce out-of-map and collision failures.
...
- Start local diffusion training from scratch with `train_scalable_diffusion_local.yaml`; save checkpoints under a new block-diffusion-specific directory.
- Run one local train batch and one validation inference first to verify block training and block rollout on real data.
...
- Compare one ablation with `block_size_chunks: 16` if block diffusion underperforms, to isolate whether block rollout helps or hurts this SMART token setup.
