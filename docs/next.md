...
- Run a SMART-Diffusion GPU smoke with `configs/train/train_scalable_diffusion_local.yaml` after the all-block Block Diffusion objective update.
...
- Watch whether target-category-only training, 6-layer decoding, clipped all-block training, monotonic block sampling, and 4-chunk block rollout reduce out-of-map and collision failures.
...
- Start local diffusion training from scratch with `train_scalable_diffusion_local.yaml`; do not continue old block-diffusion checkpoints because the loss objective and mask weighting changed.
- Run one local train batch and one validation inference first to verify all-block training and monotonic block rollout on real data.
- Validate the selected diffusion checkpoint with `configs/validation/validation_scalable_diffusion.yaml`.
- Track `train_empty_diffusion_batch`, `val_empty_diffusion_batch`, `train_block_mask_ratio`, `val_block_mask_ratio`, `val_loss`, `val_diffusion_loss`, `val_ntp_loss`, `val_mask_acc`, `val_minADE`, `val_minFDE`, `val_conflict_rate`, and `val_interaction_consistency` on target-category agents.
...
- Compare ablations with `remask_sampling: true` and `block_size_chunks: 16` if block diffusion underperforms, to isolate sampler and block-rollout effects.
