...
- Run a longer SMART-Diffusion GPU smoke with `configs/train/train_scalable_diffusion_local.yaml` after the low-variance mask and remask-sampling updates.
...
- Watch whether target-category-only training, 6-layer decoding, and remask sampling reduce out-of-map and collision failures.
