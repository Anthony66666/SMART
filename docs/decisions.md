...
- Impact: Inference samples several four-token windows, scores each by base token confidence minus decayed lane/dynamics/collision energy, commits chunk 0 of the best window, and discards chunks 1-3. The server config uses the full 128-dim/span-4 objective; the local 2000-step smoke config uses `data/valid_demo`, reduced layers/context, and a smaller span for runtime, so it should not be treated as final quality evidence.
...
- Decision: Add `smart_action_chunk_diffusion` as an additive subclass of `SMARTAutoregressiveDiffusion`. Training reuses the cadf_lite AR window objective, disables tail proposal carry/conditioning, supervises all four chunk slots equally, replays dense full-sequence SMART CE on an interval, and can add an overlap shift-consistency KL between `P_t[1:3]` and the next anchor window's chunk-0-to-2 distributions. Inference keeps an overlapping chunk memory and commits a discrete token by confidence-weighted temporal voting over `P_t[0]`, `P_{t-1}[1]`, `P_{t-2}[2]`, and `P_{t-3}[3]`.
...
- Impact: Use `configs/train/train_scalable_ar_action_chunk.yaml` for full server training, `configs/train/train_scalable_ar_action_chunk_1000.yaml` for matched local 1000-step runs, and `configs/validation/validation_scalable_ar_action_chunk.yaml` for checkpoint validation.

## Decision: Stabilize action-chunk training with SMART CE replay and overlap consistency
- Date: 2026-06-19
- Context: Server training showed weak action-chunk quality. The first action-chunk config isolated temporal ensembling but removed the original SMART full-sequence teacher-forced prior, and the training objective did not explicitly make a tail prediction from one window agree with the same future token predicted as chunk 0 in the next window.
- Decision: Keep four-token action chunks and temporal ensembling, but restore interval dense SMART CE replay and add an action-chunk-only shift-consistency auxiliary loss. The auxiliary compares overlapping logits from adjacent cadf_lite windows by scene id, agent id, and shifted chunk id, so `P_t[j+1]` is trained to agree with `P_{t+1}[j]` without feeding tail proposals back into the next window.
- Why: This keeps the method distinct from single-token SMART while directly addressing the two main train/inference gaps: weak SMART prior and untrained overlap agreement.
- Impact: Action-chunk training is slower on steps where the consistency interval is active because it builds a second adjacent window. Old action-chunk checkpoints should be treated as stale for quality comparisons.

## Decision: Add continuous diffusion-policy action chunks as the stronger action-chunk branch
- Date: 2026-06-19
- Context: Discrete ACT-style token voting preserves a clean four-token ablation, but token ids are a poor temporal-ensemble space: near-identical decoded trajectories with different codebook ids cannot average. The user asked for a diffusion-policy version after comparing ACT-style action chunks against diffusion policy.
- Decision: Add `smart_continuous_action_diffusion` as an additive subclass of `SMARTAutoregressiveDiffusion`. It reuses SMART map/history context and AR receding-horizon rollout, but trains a continuous denoising head over four-token / twenty-frame local action chunks. Inference denoises continuous chunks, converts them to world coordinates, temporally ensembles overlapping chunk predictions in continuous trajectory space, commits only chunk 0, and retokenizes the committed chunk to the nearest SMART token only for history/interface compatibility.
- Why: This keeps the method distinct from original SMART and discrete ACT while moving temporal ensembling into a geometry-aware continuous space, which is closer to diffusion-policy and VBD-style strengths for map adherence and smoothness.
- Impact: Use `configs/train/train_scalable_continuous_action_diffusion.yaml` for server training, `configs/train/train_scalable_continuous_action_diffusion_1000.yaml` for matched local 1000-step runs, and `configs/validation/validation_scalable_continuous_action_diffusion.yaml` for checkpoint validation.
